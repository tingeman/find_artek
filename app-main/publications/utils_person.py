...
def choose_person(queryset, person, user):
    ...
    print(' ')
    if len(queryset) > 1:
        print('🔍 DEBUG: Multiple matches for person: {0}'.format(name))
        print('🔍 DEBUG: Please choose correct entry:')
    else:
        print('🔍 DEBUG: Relaxed match for person: {0}'.format(name))
        print('🔍 DEBUG: Please choose:')

    for id, q in enumerate(queryset):
        print('🔍 DEBUG:    {0}:\t{1}'.format(id, fullname(q)))

    print('🔍 DEBUG:    {0}:\tCreate new person'.format('N'))  # len(queryset))

    pid = -1
    while pid < 0 or pid > len(queryset):
        pid = input('Enter choice: ')
        try:
            pid = int(pid)
        except Exception:
...
def get_person_from_string(s, user, save=False):
    ...
    if pid == 'ldap':
        ...
        if re.search('[a-zA-Z]{1}[0-9]{6}', s):
            # This is a study number...
            print("🔍 DEBUG: looking for study number")
...
def get_person(string='', person=None, initials='', person_id='', id_number='', exact=False, relaxed=False):
    ...
    if not exact:
        ...
        if not p and relaxed:
            # No relaxed match
            print('🔍 DEBUG:    No relaxed match found.')
...
def get_or_create_person_from_ldap(ldap_object=None, user=None, save=True):
    """Create Person object from pybtex instance
       ...
       ldap_object is an entry returned from an ldap search.
        ...
        In the ldap3 library, it is a dictionary-like object
        with the attributes as keys and the values as lists of strings.
    ...
    # Get person information from ldap object
    kwargs = get_person_info_from_ldap_object(ldap_object)
    ...
    q = Q()
    if id_number and initials:
        # Match entries where both id_number and initials match the provided values
        # or where one of them is empty or null (in the database), return entries
        # that match the non-null attribute.
        q = (Q(id_number=id_number) & Q(initials=initials)) |   \
            (Q(id_number=id_number) | (Q(initials='') | Q(initials__isnull=True))) |   \
            (Q(initials=initials) | (Q(id_number='') | Q(id_number__isnull=True)))
    elif id_number:
        # if only id_number is provided, match on that
        q = Q(id_number=id_number)
    elif initials:
        # if only initials is provided, match on that
        q = Q(initials=initials)
    else:
        # if neither id_number nor initials is provided, we cannot match on anything
        q = Q()

    # Now filter the Person model with the query
    # This will return a queryset of Person objects that match the query
    p = models.Person.objects.filter(q)

    # If we don't have any matches, create a new Person object
    # and set the created_by and modified_by fields to the user
    if not p:
        p = models.Person(**kwargs)
...
def update_person_from_ldap_object(person, ldap_object, user=None, save=True):
    ...
    # Get updated person information from ldap object
    kwargs = get_person_info_from_ldap_object(ldap_object)

    # Update person instance with new information
...
def get_person_info_from_ldap_object(ldap_object=None):
    """Get person information from ldap object.
    ...
    # define name parts
    name = '{0}, {1}'.format(ldap_object.sn.value, ldap_object.givenName.value)

    # Get the different name parts
    kwargs = get_full_name_kwargs(string=name)
    kwargs.update(get_relaxed_name_kwargs(string=name))

    if ldap_object.company.value == 'Studerende':
        # If we have a company 'Studerende', we assume it is a student
        kwargs['position'] = 'student'
        kwargs['id_number'] = ldap_object.name.value
    elif ldap_object.title.value:
        # If we have a title, we assume it is an employee
        kwargs['position'] = ldap_object.title.value
        kwargs['id_number'] = ldap_object.employeeID.value
        kwargs['department'] = ldap_object.department.value

    if ldap_object.initials.value:
