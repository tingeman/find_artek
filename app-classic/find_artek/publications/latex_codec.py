...
# 
# Usage:
# latexstr = b'S\\o ren g\\aa r p\\aa\ \\O{}en i \\AA en over til Sch\\"onbrunner Schlo\\ss\ og ser p\\aa\ \\ae nder'
# 
#  print unicode(latexstr,'latex')
#  >>> Søren går på Øen i Åen over til Schönbrunner Schloß og ser på ænder
#  
#  ustring = unicode(latexstr,'latex')
#  ustring.encode('latex')
#  >>> 'S{\\o}ren g{\\aa}r p{\\aa} {\\O}en i {\\AA}en over til Sch{\\"o}nbrunner Schlo{\\ss} og ser p{\\aa} {\\ae}nder'
#  
# You may also use:
#  - unicode(string,'latex+latin1')
#  - ustring.encode('latex+latin1')
# where latin1 can be replaced by any other known encoding
# 
# Original code by D. Eppstein, October 2003, taken from http://code.activestate.com/recipes/252124-latex-codec/
# 
# Code adapted by Thomas Ingeman-Nielsen, October 2012.
...
re_remove_empty_braces = re.compile(r'\{\s*\}')
re_remove_latex_cmds = re.compile(r'(?:\\[A-Za-z]+[{])(.*)(?:[}])')

# Stateless encoder/decoder
def find_latex(encoding):
    """Return the codec for 'latex'.
    """
    ...
    class LatexCodec(codecs.Codec):
        ...
        def decode(self,input,errors='strict'):
            ...
            x = u''.join(map(unicode,_unlatex(input)))
            return x, len(input)

    class LatexIncrementalEncoder(codecs.IncrementalEncoder):
        def encode(self, input, final=False):
            return LatexCodec.encode(input, self.errors)[0]

    class LatexIncrementalDecoder(codecs.IncrementalDecoder):
        def decode(self, input, final=False):
            return LatexCodec.decode(input, self.errors)[0]
...
class _unlatex:
    ...
    def __getitem__(self,n):
        ...
        p = self.pos + n
        t = self.tex
        return p < len(t) and t[p] or ''    # previously 'or None'
